...
const SignUp = () => {
    ...
    const handleVerifyOTP = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                // In a real app, you might want to update the user's name if they're signing up
                // The current backend just upserts based on phone. 
                // We'll trust the verify-otp returns the user.
                login(data.token, data.user);
                showToast('Account created! Welcome to Devaksa.');
                navigate('/');
            } else {
                showToast(data.message, 'error');
                setError(data.message);
    ...
    return (
        <div className="auth-page">
            <div className="auth-card-custom">
                <h2 style={{ textAlign: 'center', marginBottom: '20px' }}>{step === 1 ? 'Create Account' : 'Verify OTP'}</h2>
                ...
                {step === 1 ? (
                    <form onSubmit={handleSendOTP}>
                        ...
                        <p style={{ textAlign: 'center', fontSize: '14px', color: '#666' }}>
                            Already have an account? <Link to="/login" style={{ color: '#4F6F52', fontWeight: '600', textDecoration: 'none' }}>Login</Link>
                        </p>
                    </form>
                ) : (
